...
for state,details in info_dict.items():
	legend_elements.append(Patch(facecolor=details['color'],alpha=0.3, label=details['name']))
...
for state,details in info_dict.items():
	legend_elements.append(Line2D([0], [0], color=details['color'], linestyle='-', label=details['name']))
...
plt.tight_layout(); plt.savefig('plots/final/NH_clim_distrs.pdf'); plt.close()
