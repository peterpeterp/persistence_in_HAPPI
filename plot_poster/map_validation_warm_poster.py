...
plt.close('all')
fig,axes = plt.subplots(nrows=1,ncols=4,figsize=(10,10/8.),subplot_kw={'projection': ccrs.Robinson(central_longitude=0, globe=None)}, gridspec_kw = {'height_ratios':[10],'width_ratios':[7,7,1.4,1]})
#
...
for state,row in zip(['warm'],range(1)):
	...
	cbar_ax=fig.add_axes([0.91,0.05,0.1,0.9])
	cbar_ax.axis('off')
	cbar_ax.set_ylim(0,1)
	cbar_ax.set_xlim(0,1)
	cbar_ax.fill_between([0,1],[0,0],[1,1],color='w')
...
fig.tight_layout()
plt.savefig('plots/poster/map_validation_'+stat+'_warm_poster.png',dpi=300, transparent=True)
