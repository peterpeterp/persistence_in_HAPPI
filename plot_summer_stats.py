...
SH_regions=['WSA','SSA','SAF','NAU','SAU']
all_regions=['NHml']+NH_regions+SH_regions+['SHml']
...
# plot pers only
plt.close('all')
fig,axes = plt.subplots(nrows=1,ncols=2,figsize=(15,4),gridspec_kw = {'width_ratios':[3,1]})

ax=axes[0]
for scenario,color in zip(['Plus15-Future','Plus20-Future'],['sandybrown',sns.color_palette()[2]]):
	mod_mean90,mod_mean95,mod_mean99=[],[],[]
	for region,x in zip(all_regions,np.arange(len(all_regions))+0.5):
		mods90,mods95,mods99=[],[],[]
		for model,marker in zip(summer_stats.model,['*','^','o','s']):
			tmp_fu=big_dict[model][region][scenario]['JJA']['warm']
			p90_mean_fu=percentile_from_counts(tmp_fu['period_length'],tmp_fu['count'],90)
			p95_mean_fu=percentile_from_counts(tmp_fu['period_length'],tmp_fu['count'],95)
			p99_mean_fu=percentile_from_counts(tmp_fu['period_length'],tmp_fu['count'],99)

			# p90_mean_fu=np.mean(per_above_qu(tmp_fu['period_length'],tmp_fu['count'],90))
			# p95_mean_fu=np.mean(per_above_qu(tmp_fu['period_length'],tmp_fu['count'],95))
			# p99_mean_fu=np.mean(per_above_qu(tmp_fu['period_length'],tmp_fu['count'],99))

			tmp_hist=big_dict[model][region]['All-Hist']['JJA']['warm']
			p90_mean_hist=percentile_from_counts(tmp_hist['period_length'],tmp_hist['count'],90)
			p95_mean_hist=percentile_from_counts(tmp_hist['period_length'],tmp_hist['count'],95)
			p99_mean_hist=percentile_from_counts(tmp_hist['period_length'],tmp_hist['count'],99)

			# p90_mean_hist=np.mean(per_above_qu(tmp_hist['period_length'],tmp_hist['count'],90))
			# p95_mean_hist=np.mean(per_above_qu(tmp_hist['period_length'],tmp_hist['count'],95))
			# p99_mean_hist=np.mean(per_above_qu(tmp_hist['period_length'],tmp_hist['count'],99))

			ax.plot([x],[p95_mean_fu-p95_mean_hist],color=color,marker=marker)
			#ax.plot([x],[p90_mean_fu-p90_mean_hist],color=color,marker=marker)
			mods90.append(p90_mean_fu-p90_mean_hist)
			mods95.append(p95_mean_fu-p95_mean_hist)
			mods99.append(p99_mean_fu-p99_mean_hist)

		mod_mean90.append(np.mean(mods90))
		mod_mean95.append(np.mean(mods95))
		mod_mean99.append(np.mean(mods99))
	print(scenario+'********')
	print([round(zz,01) for zz in mod_mean90])
	print([round(zz,01) for zz in mod_mean95])
	print([round(zz,01) for zz in mod_mean99])
	ax.plot(np.arange(len(all_regions))+0.5,mod_mean95,color=color)


ax.set_xlim(0,len(all_regions)+1)
ax.set_xticks(np.arange(len(all_regions))+0.5)
ax.set_xticklabels(all_regions)
ax.plot([0,len(all_regions)],[0,0],'k--')
ax.set_title('95th percentile of warm persistence')
...
ax=axes[1]
for scenario,name,color in zip(['Plus15-Future','Plus20-Future'],['+1.5$^\circ$C','+2.0$^\circ$C'],['sandybrown',sns.color_palette()[2]]):
	for model,marker in zip(summer_stats.model,['*','^','o','s']):
...
plt.savefig('plots/summer_pers_changes_only.png',dpi=300)
...
# # plot all
# plt.close('all')
# fig,axes = plt.subplots(nrows=4,ncols=2,figsize=(10,6),gridspec_kw = {'width_ratios':[3,1]})
...
# ax=axes[2,0]
# for scenario,color in zip(['Plus15-Future','Plus20-Future'],['sandybrown',sns.color_palette()[2]]):
# 	tmp=summer_stats[:,scenario,:,'x90_mean_temp',:]-summer_stats[:,'All-Hist',:,'x90_mean_temp',:]
# 	tas_temp=summerTas[scenario,:,:]-summerTas['All-Hist',:,:]
# 	for region,x in zip(NH_regions,np.arange(len(NH_regions))+0.5):
# 		for model,marker in zip(summer_stats.model,['*','^','o','s']):
# 			ax.plot([x],[tmp[model,region,'mean']/float(tas_temp[region,model])],color=color,marker=marker)
# ax.plot([0,len(NH_regions)],[1,1],'k--')
#
# ax=axes[3,0]
# for scenario,color in zip(['Plus15-Future','Plus20-Future'],['sandybrown',sns.color_palette()[2]]):
# 	mod_mean=[]
...
# # plot pers only mean
# plt.close('all')
# fig,axes = plt.subplots(nrows=1,ncols=2,figsize=(9,2),gridspec_kw = {'width_ratios':[3,1]})
#
# ax=axes[0]
# for scenario,color in zip(['Plus15-Future','Plus20-Future'],['sandybrown',sns.color_palette()[2]]):
# 	mod_mean=[]
...
# 		for model,marker in zip(summer_stats.model,['*','^','o','s']):
# 			tmp_fu=big_dict[model][region][scenario]['JJA']['warm']
# 			qu_fu=percentile_from_counts(tmp_fu['period_length'],tmp_fu['count'],'mean')
# 			p90_mean_fu=np.mean(per_above_qu(tmp_fu['period_length'],tmp_fu['count'],90))
#
# 			tmp_hist=big_dict[model][region]['All-Hist']['JJA']['warm']
# 			qu_hist=percentile_from_counts(tmp_hist['period_length'],tmp_hist['count'],'mean')
# 			p90_mean_hist=np.mean(per_above_qu(tmp_hist['period_length'],tmp_hist['count'],90))
#
# 			ax.plot([x],[qu_fu-qu_hist],color=color,marker=marker)
# 			#ax.plot([x],[p90_mean_fu-p90_mean_hist],color=color,marker=marker)
...
#
# ax.set_xticklabels(NH_regions)
# plt.savefig('plots/summer_pers_changes_mean.png',dpi=300)
#
#
# #
# #
