...
def plot_driver_column(ax,x0,state,corWi,letter):
	...
	x += 1
	rel_diff_corWi = (summary['Plus20-Future',:,state,corWi,:,'corrcoef'+details[corWi],'JJA'] - summary['All-Hist',:,state,corWi,:,'corrcoef'+details[corWi],'JJA']) #/ summary['All-Hist',:,state,corWi,:,'corrcoef'+details[corWi],'JJA'] * 100
	rel_diff_corWi[np.isfinite(rel_diff_corWi)==False] = np.nan
	c_range = plot_model_column(ax,x,rel_diff_corWi,label = 'change\nin cor '+corWi+' '+corWi_dict[corWi]['units'], cmap=corWi_dict[corWi]['cmap'],c_range='maxabs')
...
##############################################################################
'''
def plot_legend(ax,x,y,obs=None,icons=None):
	xx,yy = x+1.5,y
	patches = []
	for model in model_shifts.keys():
		x_shi,y_shi = model_shifts[model]
	...
	if icons is not None:
		...
		y-=1
		for hatch,name in zip(['\\\\\\\\\\','////'],['correlation\nalpha = 0.1','change\nalpha = 0.01']):
...
plt.close('all')
fig,ax = plt.subplots(nrows=1,figsize = (10,6.4), dpi=600)
ax.axis('off')
...
plot_legend(ax,20,15.5,'EOBS', icons={'EKE':'EKE','SPI3':'SPI3','rain':'\nchange in\nnumber of\nrain/dry days'})
ax.set_ylim(-1.5,17.5)
ax.set_xlim(0.5,23)
plt.tight_layout()
plt.savefig('plots/table_driver_'+'dry'+'_test.pdf')
