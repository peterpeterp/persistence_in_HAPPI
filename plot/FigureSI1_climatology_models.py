...
for style,state in zip(['tas','tas','pr','pr','cpd','cpd'],['warm','cold','dry','wet','dry-warm','wet-cold']):
	...
	for dataset,row in zip(['MIROC5','NorESM1','ECHAM6-3-LR','CAM4-2degree'],range(4)):
		for stat,ax,col in zip(['mean','qu_95'],axes[row,:],range(2)):
			tmp=sum_meanQu[style][dataset]
			to_plot=(tmp['All-Hist'][season][state][stat])
			crange=color_range[state][stat]
			im=ax.pcolormesh(tmp.lon,tmp.lat,to_plot,vmin=crange[0],vmax=crange[1],cmap=cmap,transform=ccrs.PlateCarree());
