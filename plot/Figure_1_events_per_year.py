...
for model in ['MIROC5','NorESM1','ECHAM6-3-LR','CAM4-2degree']:
	...
	for region in ['ENA','CAS','NAS','CAM','CNA','NEU','WAS','TIB','CGI','MED','WNA','ALA','CEU','EAS']:
		grid_dict[model][region] = len(np.where(masks[region]>0)[0])
	grid_dict[model]['NHml']=len(masks.lat[(masks.lat>=35) & (masks.lat<=60)])*masks.lon.shape[0]

grid_dict['HadGHCND']={}
masks=da.read_nc('masks/srex_mask_73x97.nc')
for region in ['ENA','CAS','NAS','CAM','CNA','NEU','WAS','TIB','CGI','MED','WNA','ALA','CEU','EAS']:
	grid_dict['HadGHCND'][region] = len(np.where(masks[region]>0)[0])
grid_dict['HadGHCND']['NHml']=len(masks.lat[(masks.lat>=35) & (masks.lat<=60)])*masks.lon.shape[0]
...
def distrs(subax,region,arg1=None,arg2=None,arg3=None,arg4=None):
	...
	for style,state,color in zip(arg2,arg3,arg4):
		...
		for dataset,i in zip(['MIROC5','NorESM1','ECHAM6-3-LR','CAM4-2degree'],range(4)):
			...
			count_h=np.array([np.sum(tmp_h['count'][ii:])/float(10*100*grid_dict[model][region]) for ii in range(len(tmp_h['count']))])
	...
	count_h=np.array([np.sum(tmp_h['count'][ii:])/float(56*grid_dict['HadGHCND'][region]) for ii in range(len(tmp_h['count']))])
