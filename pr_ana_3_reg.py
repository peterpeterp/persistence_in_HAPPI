...
def get_regional_distribution(regions,model,scenarios=['All-Hist','Plus20-Future'],add_name=''):
	region_dict={}
	for region in regions.keys():
		...
		for scenario in scenarios:
			...
			for season in ['MAM','JJA','SON','DJF']:
				...
				region_dict[region][scenario][season]={'dry':{},'wet':{}}
			...
			for season in ['MAM','JJA','SON','DJF']:
				if len(tmp[season])>5:
					for state,state_name in zip([-1,1],['dry','wet']):
	...
	output = open('data/'+model+'/pr_'+model+'_regional_distrs_'+add_name+'.pkl', 'wb')
