...
colors=['darkred','darkorange','magenta']#+sns.color_palette("colorblind", 4)

# ---------------------------- changes
def legend_plot(subax):
	subax.axis('off')
	for style,color in zip(['warm persistence','dry persistence','warm+dry persistence'],colors):
...
def distrs(subax,region,arg1=None,arg2=None,arg3=None):
	...
	for style,state,color in zip(['tas','pr','cpd'],['warm','dry','dry-warm'],colors):
		tmp_h=big_dict[style][region]['All-Hist'][season][state]
		count_h=np.array([np.sum(tmp_h['count'][ii:])/float(np.sum(tmp_h['count'])) for ii in range(len(tmp_h['count']))])
		subax.plot(range(1,len(count_h)+1),count_h,color=color)
		subax.annotate('   '+region, xy=(0, 0), xycoords='axes fraction', fontsize=10,xytext=(-5, 5), textcoords='offset points')


fig,ax_map=srex_overview.srex_overview(distrs,axis_settings,polygons=polygons,reg_info=all_regs,arg1='summer',arg2='warm', x_ext=[-20,80], y_ext=[15,85], small_plot_size=0.25, legend_plot=legend_plot, legend_pos=[64,75], title='persistence distributions in JJA',size=6)
