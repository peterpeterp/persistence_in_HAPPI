...
for scenario in scenario_list:
	...
	for run in run_list:
		if scenario in ['All-Hist']:
			...
			out_file_name_tmp=glob.glob(tmp_path+run+'/*')[0].split('/')[-1].split(run)[0]+run+'_tmp.nc'
			command='cdo -O mergetime '+tmp_path+run+'/* '+out_path+scenario+'/'+out_file_name_tmp
			print command
			os.system(command)
			os.system('cdo -O -selyear,2006/2016 '+out_path+scenario+'/'+out_file_name_tmp+' '+out_path+scenario+'/'+out_file_name)
			os.system('rm '+out_path+scenario+'/'+out_file_name_tmp)
		else:
			out_file_name=glob.glob(tmp_path+run+'/*')[0].split('/')[-1].split(run)[0]+run+'.nc'
			command='cdo -O mergetime '+tmp_path+run+'/* '+out_path+scenario+'/'+out_file_name
			os.system(command)			
