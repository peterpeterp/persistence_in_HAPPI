...
for scenario in ['All-Hist','Plus20-Future','Plus15-Future']:
    ...
    if os.path.isfile(out_file)==False:
        ...
        stat_period_id=da.DimArray(axes=[np.asarray(runs),np.asarray(range(period_number_limit),np.int32),qu_90.lat,qu_90.lon],dims=['run','ID','lat','lon'])
            		...
            		if np.sum(summer_events[:,y,x])!=0:
            			...
            			if len(summer_ids)!=0:
            				...
            				if len(summer_ids)<=period_number_limit:
            					...
            					stat_period_id[run,0:len(summer_ids)-1,lat,lon]=event_ids
    ...
    ds=da.Dataset({'90X_cum_heat':stat_Xpers_cum_heat,'90X_hot_shift':stat_Xpers_hot_shift,'90X_hot_temp':stat_Xpers_hot_temp,'90X_mean_temp':stat_Xpers_mean_temp,'TXx_in90Xpers':stat_TXx_in_Xpers,'period_id':stat_period_id})
