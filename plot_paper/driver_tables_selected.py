...
with PdfPages('plots/table_driver_selected.pdf') as pdf:
	for corWi in selection.keys():
		...
		for state in selection[corWi].keys():
			regions = {key:i+start for i,key in enumerate(selection[corWi][state])}
			start += len(selection[corWi][state]) + 1
			...
			c_range = plot_model_column(ax,x,var=cor,\
								...
								label='', cmap='PuOr', c_range='maxabs', signi_lvl=0.1)
			if state != 'warm':
			...
			for region,y_reg in regions.items():
				...
				ax.text(0.2,y_reg,region,va='center',weight='bold',color='k')
