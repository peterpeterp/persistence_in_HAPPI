		...
		for run in run_list:
			...
			if len(glob.glob(tmp_path+run+'/*'))>0:
				...
				if os.path.isfile(pr_state_file) == False:
					...
					prsfc.precip_to_index(land_file,pr_state_file,overwrite=True,unit_multiplier=86400,thresholds=[1,5,10])
				...
				if os.path.isfile(pr_period_file) == False:
					prsfc.get_persistence(pr_state_file,states_to_analyze={-1:'dry',1:'wet',5:'5mm',10:'10mm'},overwrite=True)
				gc.collect()
