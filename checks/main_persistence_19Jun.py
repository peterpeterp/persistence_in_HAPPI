...
for scenario,selyears in zip(['Plus20-Future','Plus15-Future','All-Hist'],['2106/2115','2106/2115','2006/2015']):
	if scenario==sys.argv[2]:
		...
		for run in run_list:
			...
			if len(glob.glob(working_path+scenario+'/'+glob.glob(tmp_path+run+'/*')[0].split('/')[-1].split(run)[0]+run+'*'))<2 or True:
				...
				prsfc.temp_anomaly_to_ind(anom_file,state_file,overwrite=True)
				...
				prsfc.get_persistence(state_file,raw_file.replace('.nc','_period_19Jun.nc'),overwrite=True,eke_file=eke_file,spi_file=spi_file)
