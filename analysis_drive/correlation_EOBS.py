...
for pers_file in [working_path+scenario+'/rr_0.50deg_reg_merged_period_5mm.nc',working_path+scenario+'/rr_0.50deg_reg_merged_period_dry.nc']:
...
if 'period_monthly_index' in data.keys():
	...
	da.Dataset(cor).write_nc(working_path+scenario+'/cor_'+corWith_name+'_'+'_'.join([model,scenario])+'_dry.nc')
	cor_longest['corrcoef'].persistence = pers_file
	cor_longest['corrcoef'].correlated_with = corWith_file
	da.Dataset(cor_longest).write_nc(working_path+scenario+'/corLongest_'+corWith_name+'_'+'_'.join([model,scenario])+'_dry.nc')
